    ...
    ws.on('connection', function connection(ws)
    {
        ...
        function JoinOrCreateRoom(_inRoomName, _inID)
        {
          ...
          if(!rooms.get(_inRoomName).roomClients.has(_inID))
          {
              ...
              {
                try { rooms.get(_inRoomName).roomClients.get(rooms.get(_inRoomName).roomMasterWSID).ws.send(FMEventEncode("OnClientConnectedEvent", wsid)); }
                catch (err) { console.warn("Failed to send to room master:", err.message); }
              }
          }
            ...
            if (isBinary === false)
            {
                try
                  ...
                  if (message.includes("fmevent:"))
                  {
                      ...
                      if (decodedResult.length < 3 || decodedResult[0] !== "fmevent") return;
                      ...
                      switch(decodedType)
                      {
                        ...
                        case 'networkType':
                            ...
                            {
                              //this should prevent infinite loop?
                              ws.send(FMEventEncode("OnJoinedLobbyEvent", clients.get(wsid).roomName));
                            }
                ...
                {
                    console.warn("Error in message handler:", err.message);
                }
            }
            else
            {
